...
const WeightTrackingPage = () => {
  ...
  const [weightEntries, setWeightEntries] = useState<WeightEntry[]>([
    { id: '1', date: '2024-08-05', weight: 175.2, bodyFatPct: 15.2, notes: 'Morning weight' },
    ...
    { id: '5', date: '2024-07-28', weight: 177.0, bodyFatPct: 16.0, notes: 'Post vacation' },
  ...
  const handleSaveWeight = () => {
    ...
    const weightData: WeightEntry = {
      ...
      bodyFatPct: formData.bodyFatPct ? parseFloat(formData.bodyFatPct) : undefined,
      notes: formData.notes || undefined
    };

    if (editingEntry) {
      setWeightEntries(prev => 
        prev.map(entry => entry.id === editingEntry.id ? weightData : entry)
          .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
      );
    } else {
      setWeightEntries(prev => 
        [weightData, ...prev].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
      );
    }
      ...
      <Box sx={{ 
        ...
        <Card sx={{ flex: 1 }}>
          <CardContent sx={{ textAlign: 'center' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
              ...
              <Typography 
                variant="caption" 
                color={weightChange > 0 ? 'error.main' : weightChange < 0 ? 'success.main' : 'text.secondary'}
                fontWeight="medium"
              >
        ...
        <Card sx={{ flex: 1 }}>
          <CardContent sx={{ textAlign: 'center' }}>
            ...
            <Typography 
              ...
              fontWeight="medium"
            >
              {weightToGoal > 0 ? `${weightToGoal.toFixed(1)} lbs to go` : 'Goal achieved!'}
            </Typography>
          </CardContent>
        </Card>
        ...
        <Card sx={{ flex: 1 }}>
          <CardContent sx={{ textAlign: 'center' }}>
            <TimelineIcon sx={{ 
              fontSize: 40, 
              color: trend === 'decreasing' ? 'success.main' : trend === 'increasing' ? 'error.main' : 'text.secondary',
              mb: 1 
            }} />
            <Typography variant="h6" fontWeight="bold">
              {trend === 'decreasing' ? 'Losing' : trend === 'increasing' ? 'Gaining' : 'Stable'}
            </Typography>
          ...
          <Box sx={{ height: 300 }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                <Tooltip />
                <ReferenceLine y={goalWeight} stroke="#FF9800" strokeDasharray="5 5" label="Goal" />
                <Line 
                  type="monotone" 
                  dataKey="weight" 
                  stroke="#2196F3" 
